...
export function checkRegister(values) {
  ...
  if (
    !validator.isStrongPassword(values.firstPassword, {
      minlength: 8,
      minLowerCase: 1,
      minUppercase: 1,
      minNumbers: 1,
    ...
    errors["firstPassword"] =
      "Votre mot de passe doit contenir 8 caractères, 1 majuscule, 1 minuscule, 1 chiffre.";
  }
  if (!validator.isLength(values.username, { min: 3, max: 50 })) {
    errors["username"] =
      "Votre nom d'utilisateur doit être compris entre 3 et 50 carractères.";
  }
  return errors;
}
